...
-(instancetype)initWithFrame:(CGRect)frame
{
    if(self = [super initWithFrame:frame])
    {
        ...
        UIView *roundView = [[UIView alloc]initWithFrame:CGRectMake(ScreenWidth/2-25, -20, 50, 50)];
        ...
        //设置默认背景图片
        [addBtn setBackgroundImage:[UIImage imageNamed:@"tabbar_icon_default_pt"] forState:UIControlStateNormal];
        //设置按下时背景图片
        [addBtn setBackgroundImage:[UIImage imageNamed:@"tabbar_icon_default_pt"] forState:UIControlStateHighlighted];
        //添加响应事件
...
-(void)layoutSubviews
{
    ...
    //设置“+”按钮的位置
    self.addButton.center = CGPointMake(self.center.x, self.bounds.size.height * 0.5 - 1.5 * AddButtonMargin);
    ...
    addLbl.text = @"盘Ta";
    addLbl.font = [UIFont systemFontOfSize:13];
    addLbl.textColor = Tab_Color_Normal;
    [addLbl sizeToFit];
    //设置“添加”label的位置
    addLbl.center = CGPointMake(self.addButton.center.x, CGRectGetMaxY(self.addButton.frame) + 0.5 * AddButtonMargin + 0.5);
